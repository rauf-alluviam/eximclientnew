...
class ModuleAssignmentFrontendTester {
  ...
  getCurrentUser() {
    try {
      ...
      if (userData) {
        ...
        this.log(
          `Current user: ${this.currentUser.name} (${this.currentUser.ie_code_no})`
        );
        return this.currentUser;
      } else {
        this.log("No user data found in cookies", "error");
        return null;
      }
    } catch (error) {
  ...
  testLocalStorageUserData() {
    ...
    if (userData.assignedModules) {
      this.log(
        `✅ User has assignedModules: ${JSON.stringify(
  ...
  testModuleAccessUtilities() {
    ...
    try {
      ...
      if (typeof window !== "undefined" && window.moduleAccessUtils) {
        const { getUserAssignedModules, hasModuleAccess } =
          window.moduleAccessUtils;

        const assignedModules = getUserAssignedModules();
        this.log(
          `✅ getUserAssignedModules returned: ${JSON.stringify(
            assignedModules
          )}`
        );
      ...
      this.log(
        `❌ Error testing module access utilities: ${error.message}`,
        "error"
      );
      return false;
    }
  ...
  async testUserProfileAPI() {
    ...
    try {
      const response = await fetch(
        `${this.API_BASE_URL}/api/customer/profile`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        if (data.data && data.data.assignedModules) {
          this.log(
            `✅ API returned assignedModules: ${JSON.stringify(
        ...
        this.log(
          `❌ User profile API failed: ${response.status} ${response.statusText}`,
          "error"
        );
        return false;
      }
  ...
  testNavigationAccess() {
    ...
    if (!this.currentUser || !this.currentUser.assignedModules) {
      this.log(
        "❌ Cannot test navigation - no user or assigned modules",
        "error"
      );
      return false;
    }
    ...
    this.log(
      `📊 Accessible modules: ${accessibleModules}/${assignedModules.length}`
    );
  ...
  async testRealTimeUpdates() {
    ...
    try {
      if (
        ...
        window.moduleAccessUtils &&
        window.moduleAccessUtils.refreshUserData
      ) {
        ...
        if (updatedUser) {
          this.log(
            `✅ Updated modules: ${JSON.stringify(updatedUser.assignedModules)}`
          );
          return true;
        } else {
  ...
  testErrorHandling() {
    ...
    try {
      // Test with invalid user data stored in cookies
      const { getJsonCookie, setJsonCookie } = require("./cookies");
      const originalData = getJsonCookie("userData");
      setJsonCookie("userData", "invalid_json");
      ...
      if (originalData) {
        setJsonCookie("userData", originalData);
      }
  ...
  testModuleDebugging() {
    ...
    if (typeof window !== "undefined" && window.ModuleDebugger) {
      ...
      this.log(
        "⚠️  ModuleDebugger not available (normal in production)",
        "warning"
      );
    ...
    const tests = [
      {
        ...
        fn: () => this.testLocalStorageUserData(),
      },
      {
        name: "Module Access Utilities",
        fn: () => this.testModuleAccessUtilities(),
      },
  ...
  generateReport() {
    const report = {
      ...
      totalTests: this.results.filter((r) => r.message.includes('Test "'))
        .length,
      passedTests: this.results.filter((r) => r.message.includes("passed"))
        .length,
      failedTests: this.results.filter((r) => r.message.includes("failed"))
        .length,
