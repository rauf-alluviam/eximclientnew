...
const ImportAnalytics = () => {
  ...
  const fetchHsCodes = useCallback(async () => {
    ...
    try {
      ...
      const uniqueHsCodes = [
        ...new Set(res.data.filter((hsCode) => hsCode && hsCode.trim() !== "")),
      ];
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      const uniqueSuppliers = [
        ...new Set(
          res.data.filter((supplier) => supplier && supplier.trim() !== "")
        ),
      ];
  ...
  const fetchAnalytics = async (search = {}) => {
    ...
    try {
      const [costResponse, suppliersResponse] = await Promise.all([
        ...
        axios.get(
          `${process.env.REACT_APP_API_STRING}/analytics/best-suppliers`,
          {
            params: {
  ...
  const handleSort = (field, data) => {
    ...
    const sortedData = [...data].sort((a, b) => {
      ...
      if (
        ["avg_per_kg_cost", "min_avg_per_kg_cost", "shipment_count"].includes(
          field
        )
      ) {
    ...
    <th
      ...
      onClick={() => {
        if (field.includes("best_")) {
          setBestSuppliers(
            handleSort(field.replace("best_", ""), bestSuppliers)
          );
        } else {
        ...
        >
          <div style={{ flex: "1", minWidth: "200px" }}>
            <Autocomplete
              ...
              isOptionEqualToValue={(option, value) => {
                if (value === "All HS Codes" && option === "All HS Codes")
                  return true;
                if (
                  value !== "All HS Codes" &&
                  option !== "All HS Codes" &&
          ...
          <div style={{ flex: "1", minWidth: "200px" }}>
            <Autocomplete
              ...
              onChange={(event, newValue) => {
                const value =
                  newValue === "All Suppliers" ? "" : newValue || "";
              ...
              loading={loadingDropdowns}
              isOptionEqualToValue={(option, value) => {
                if (value === "All Suppliers" && option === "All Suppliers")
                  return true;
                if (
                  value !== "All Suppliers" &&
  ...
  const calculateTotals = () => {
    ...
    const totalShipments = costAnalytics.reduce(
      (sum, item) => sum + item.shipment_count,
      0
    );
    const weightedAvgCost =
      costAnalytics.reduce(
        (sum, item) => sum + item.avg_per_kg_cost * item.shipment_count,
        0
      ) / totalShipments;
              ...
              >
                <thead>
                  ...
                  >
                    ...
                    />
                    <SortableHeader
                      field="best_min_avg_per_kg_cost"
                      label="Min Avg Cost/KG"
                      align="right"
                    />
                    <SortableHeader
                      field="best_shipment_count"
                      label="Shipments"
                      align="right"
                    />
                  </tr>
