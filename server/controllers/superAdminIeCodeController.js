...
export const removeIeCodeFromUser = async (req, res) => {
  try {
    ...
    if (user.role === 'admin' && user.ie_code_assignments.length === 1) {
      return res.status(400).json({
        success: false,
        message: "Cannot remove the last IE code from an admin user. Demote the user first."
      });
    }
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        if (user.ie_code_assignments?.some(a => a.ie_code_no === ieCodeNo.toUpperCase())) {
          errors.push({
            userId: user._id,
            name: user.name,
        ...
        user.addIeCodeAssignment(ieCodeNo, customerKyc.name_of_individual, req.superAdmin);
        await user.save();
        successCount++;
